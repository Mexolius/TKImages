...
def execute_sequence():
    ...
    while(True):
        ...
        for id in children:
            parsed[-1][dpg.get_item_label(id)]=dpg.get_value(id)
    ...
    for p in parsed:
        ...
        time.sleep(1)
        success_num+=1
        dpg.set_value("progress_bar",success_num/seq_len)
...
with dpg.window(tag="main_window"):
    with dpg.group(tag="menu",horizontal=True):
        dpg.add_button(label="Execute",callback=execute_sequence)
    ...
    with dpg.node_editor(callback=link_callback, delink_callback=delink_callback,tag="node_editor",tracked=True) as node_editor:
